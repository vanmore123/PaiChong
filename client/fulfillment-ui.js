(() => {
  ...
  const escape = (v = '') => String(copy(v)).replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;').replaceAll("'", '&#039;');
  ...
  const states = {
    awaiting_payment: ['请确认验宠费用', '司机已出具确认总价，扣除宠物运输检疫费后确认剩余费用。', 'orange'],
  ...
  function markup(order, role = 'user') {
    ...
    return `<section class="fulfillment-panel" aria-label="履约与尾款"><div class="fulfillment-heading"><h3>${ended ? '原验宠费用记录' : '验宠与费用确认'}</h3><span class="fulfillment-chip">${ended ? '已结束 · 无需支付' : invoice.status === 'paid' ? '已确认' : '待用户确认'}</span></div>
      <dl class="fulfillment-prices"><div><dt>司机确认总价</dt><dd>${money(invoice.total)}</dd></div><div><dt>已付宠物运输检疫费抵扣</dt><dd>− ${money(invoice.deposit)}</dd></div><div class="fulfillment-total"><dt>${ended ? (invoice.status === 'paid' ? '原已付尾款' : '原尾款（已关闭）') : invoice.status === 'paid' ? '已付尾款' : '待付尾款'}</dt><dd>${money(invoice.amount)}</dd></div></dl>
      <p class="fulfillment-note">验宠说明：${escape(copy(f.inspection.note))}</p>
      ${role === 'user' && invoice.status !== 'paid' && f.stage === 'awaiting_payment' ? `<form data-fulfillment-form="balance"><label class="fulfillment-check"><input name="consent" type="checkbox" required />我已核对确认总价 ${money(invoice.total)}，同意宠物运输检疫费抵扣，知悉体验版支付不扣款。</label><button class="primary-button" type="submit">${invoice.amount === 0 ? '确认费用，无需补款' : `确认支付 ${money(invoice.amount)}`}</button><p class="form-message" role="alert" hidden></p></form>` : ''}
