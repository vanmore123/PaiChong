(() => {
  ...
  async function renderProfile() {
    ...
    if (state.role === 'user' && window.PaichongProfiles) {
      ...
      if (window.PaichongProtection) byId('mini-page').querySelector('.mini-menu-card').insertAdjacentHTML('beforeend', `<button type="button" data-protection-guide>${icon('shield-check')}<span>保险与服务协议</span>${icon('chevron-right')}</button>`);
