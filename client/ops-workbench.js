(() => {
  ...
  function showError(message = '') {
    ['nodes-workbench-error', 'routes-workbench-error'].forEach((id) => { byId(id).textContent = copy(message); byId(id).hidden = !message; });
  ...
  function renderRouteCreate() {
    ...
    byId('route-create-form').addEventListener('submit', (event) => {
      ...
      confirm('创建这条线路？', `${cities.join(' → ')}，${dateTimeLabel(departureAt)}（北京时间）发车。${capacity} 个笼位，至少 ${minOrders} 单发车。`, () => mutate('/api/ops/routes', { name, cities, departureAt, arrivalAt, capacity, minOrders, maxWaitingHours }, '线路已创建，可以开始编单', (result) => { state.routeId = result.route?.id || result.item?.id || result.id || ''; state.creatingRoute = false; state.routeScreen = 'detail'; }), false, '确认创建线路');
    });
  }
  ...
  function renderRouteDetail() {
    ...
    const dispatchReason = route.dispatchBlockedReason || (!editable ? '当前线路已派车，不能重复派车或修改编单。' : !route.driverId || !route.vehicleId ? '先分配司机与车辆，再执行派车。' : routeOrders.length < Number(route.minOrders) ? `还需 ${Number(route.minOrders) - routeOrders.length} 单达到最低发车数量。` : '发车条件已满足，请核对订单、司机与车辆。');
    ...
    byId('route-workbench-detail').innerHTML = `<div class="workbench-card-heading"><div><p class="eyebrow">${escape(route.id)}</p><h3>${escape(copy(route.name))}</h3></div>${statusTag(route.status, route.status === '凑单中')}</div><p class="route-plan-intro">${escape((route.cities || []).join(' → '))}</p>
      ...
      <div class="route-section"><h4>司机与车辆</h4><form class="route-assign-form" id="route-assign-form"><label class="field-label" for="route-driver">司机<select id="route-driver" required${editable ? '' : ' disabled'}>${resourceOptions(state.resources.drivers, route.driverId, 'driver', route.driverName)}</select></label><label class="field-label" for="route-vehicle">车辆<select id="route-vehicle" required${editable ? '' : ' disabled'}>${resourceOptions(state.resources.vehicles, route.vehicleId, 'vehicle', route.vehiclePlate)}</select></label><div class="route-resource-actions"><button class="outline-button" type="submit"${editable ? '' : ' disabled'}>${icon('truck')}<span>${route.driverId ? '更新司机与车辆' : '分配司机与车辆'}</span></button>${editable && (route.driverId || route.vehicleId) ? '<button class="quiet-button" id="route-unassign" type="button">取消资源分配</button>' : ''}</div></form><p class="workbench-helper">同一资源不能在重叠的运输时间内分配给多条线路。车辆容量须覆盖线路容量。</p></div>
      <div class="route-section"><div class="dispatch-callout"><p><strong>${isDispatched(route) ? '已执行派车' : '派车前最后核对'}</strong><br>${escape(copy(dispatchReason))}</p><button class="pay-button" id="route-dispatch" type="button"${canDispatch ? '' : ' disabled'}>${icon('truck')}<span>派车</span></button></div></div>`;
    ...
    byId('route-assign-form').addEventListener('submit', (event) => {
      ...
      confirm('确认司机与车辆分配？', `${copy(route.name)} 将分配给 ${driver.name}，车辆 ${vehicle.plate}。请与司机核对接宠和出发安排。`, () => mutate(`/api/ops/routes/${encodeURIComponent(route.id)}/assign`, { driverId, vehicleId }, '司机与车辆已分配'), false, '确认分配');
    });
    byId('route-unassign')?.addEventListener('click', () => {
      confirm('取消这条线路的资源分配？', `${copy(route.name)} 将释放当前司机与车辆的时间占用，编入的订单和合作点预约保持不变。重新分配资源后才能派车。`, () => mutate(`/api/ops/routes/${encodeURIComponent(route.id)}/assign`, { driverId: '', vehicleId: '' }, '已取消资源分配，可以重新选择司机车辆'), true, '确认取消分配');
    });
    byId('route-dispatch').addEventListener('click', () => {
      if (!canDispatch) return;
      confirm('确认执行派车？', `${copy(route.name)} · ${routeOrders.length} 单。派车后不能再修改编单或司机车辆；已到点订单可继续完成离点交接。`, () => mutate(`/api/ops/routes/${encodeURIComponent(route.id)}/dispatch`, {}, '派车成功，请在合作点完成到点与离点交接'), false, '确认派车');
    });
  }
  ...
  function resourceOptions(resources, selectedId, type, selectedName) {
    ...
    return initial + missingSelected + resources.map((resource) => {
      ...
      const label = type === 'driver' ? `${resource.name} · ${resource.phone || '司机'}` : `${resource.plate} · ${resource.capacity} 个笼位`;
