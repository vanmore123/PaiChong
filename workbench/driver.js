(() => {
  ...
  function detail(order) {
    ...
    main.innerHTML = `<section class="driver-detail-hero"><p>${escape(order.id)}</p><div>${icon(order.petType === '犬' ? 'dog' : 'cat')}<h1>${escape(order.petName)}的出行</h1></div><h2>${escape(order.fromCity)} → ${escape(order.toCity)}</h2><span class="fulfillment-chip">${escape(labels[f?.stage] || '待验宠')}</span></section>${nextAction(order)}${ui.markup(order, 'driver')}
      <section class="driver-contact-card"><h2>本次接送信息</h2><dl><div><dt>宠物资料</dt><dd>${escape(order.petType)} · ${escape(order.breed)} · ${escape(order.weight)}kg</dd></div><div><dt>接送方式</dt><dd>${escape(order.bookingSelection?.serviceLabel || order.serviceType || '按订单安排')}</dd></div><div><dt>出发交接点</dt><dd>${escape(order.assignedNode?.name || '待分配')}${order.assignedNode?.address ? `<br>${escape(order.assignedNode.address)}` : ''}</dd></div><div><dt>司机 / 车辆</dt><dd>${escape(order.transport?.driverName)} · ${escape(order.transport?.vehiclePlate)}</dd></div><div><dt>${order.bookingSelection?.pickupMode === 'node' ? '送宠人 / 送宠地址' : '寄件人 / 接宠地址'}</dt><dd>${escape(copy(order.contactName))} · ${escape(order.contactPhone)}<br>${escape(copy(order.pickupAddress || '待确认'))}</dd></div><div><dt>${order.bookingSelection?.deliveryMode === 'node' ? '取宠人 / 意向网点' : '收件人 / 送达地址'}</dt><dd>${escape(copy(order.recipientName))} · ${escape(order.recipientPhone)}<br>${escape(copy(order.recipientAddress))}${order.bookingSelection?.deliveryMode === 'node' ? '<br>到达交接安排待运营确认' : ''}</dd></div><div><dt>照护说明</dt><dd>${escape(copy(order.careNote || '暂无特殊照护说明'))}</dd></div></dl></section>
