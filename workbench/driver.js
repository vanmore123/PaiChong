(() => {
  ...
  const labels = { awaiting_payment: '待尾款确认', ready: '费用已确认', in_transit: '运输中', arrived: '待签收', exception: '异常待处理', delivered: '已签收' };
  ...
  function tasks() {
    const active = state.orders.filter((o) => o.fulfillment?.stage !== 'delivered'), done = state.orders.length - active.length;
    const shown = state.filter === 'done' ? state.orders.filter((o) => o.fulfillment?.stage === 'delivered') : active;
    main.innerHTML = `<div class="driver-greeting"><div><p>司机工作台</p><h1>你好，${escape(state.session.name)}</h1></div><span>${icon('van')}</span></div><div class="driver-metrics"><div><strong>${active.filter((o) => !o.fulfillment?.inspection).length}</strong><span>待验宠</span></div><div><strong>${active.filter((o) => o.fulfillment?.stage === 'awaiting_payment').length}</strong><span>待尾款</span></div><div><strong>${active.filter((o) => o.fulfillment?.stage === 'exception').length}</strong><span>待处理异常</span></div></div><p class="driver-demo-note">仅显示已派给你的任务 · 所有操作均为虚拟演示</p><div class="mini-order-filters"><button data-filter="active" class="${state.filter === 'active' ? 'is-active' : ''}" aria-pressed="${state.filter === 'active'}">进行中 ${active.length}</button><button data-filter="done" class="${state.filter === 'done' ? 'is-active' : ''}" aria-pressed="${state.filter === 'done'}">已完成 ${done}</button></div><div class="driver-task-list">${shown.length ? shown.map(card).join('') : `<div class="driver-empty"><img src="./assets/v5/illustrations/empty-orders.png" alt="" /><h2>${state.filter === 'done' ? '还没有已完成任务' : '暂时没有待处理任务'}</h2><p>经营者完成编线、分配司机和模拟派车后，任务会出现在这里。</p></div>`}</div>`;
  ...
  function nextAction(order) {
    ...
    if (!f) {
      ...
      return `<section class="driver-action-card"><span class="driver-step">下一步 · 现场验宠</span><h2>核对毛孩子与交接资料</h2><p>锁价后本轮不支持重复改价，请先核对费用。</p>${form('inspection', `${checkbox('identity', '宠物身份与材料一致')}${checkbox('cage', '笼具状态与固定情况已核对')}${checkbox('handoff', '现场交接情况已核对，可继续履约')}<label>验宠确认总价（元）<input name="lockedPrice" type="number" min="${order.deposit.amount}" max="100000" step="0.01" required value="${escape(order.confirmedPrice ?? order.proposedPrice)}" /></label><label>验宠与费用说明<textarea name="note" rows="3" maxlength="300" required placeholder="记录核对情况；如价格变化，请说明原因"></textarea></label>`, '确认验宠并提交总价')}</section>`;
    }
    if (f.stage === 'awaiting_payment') return '<section class="driver-action-card"><span class="driver-step">等待用户</span><h2>总价已提交，等待确认尾款</h2><p>用户确认并模拟付清费用后，经营者才能办理离点交接。司机不能代替用户付款。</p></section>';
    if (f.stage === 'ready') return `<section class="driver-action-card"><span class="driver-step">下一步 · 出发准备</span><h2>${handoff === 'departed' ? '交接完成，可以准备出发' : '等待合作点离点交接'}</h2>${handoff === 'departed' ? form('depart', checkbox('ready', '已完成宠物、笼具与车辆交接，确认模拟出发'), '确认模拟出发') : '<p>费用已确认，请经营者办理离点。完成后刷新即可登记出发。</p>'}</section>`;
    if (f.stage === 'in_transit') {
      ...
      return `<section class="driver-action-card"><span class="driver-step">下一步 · 更新节点</span><h2>记录这一路的照护</h2><p>上次模拟节点：${escape(f.currentCity)}</p>${form('checkpoint', `<label>本次到达城市<select name="city" required>${remaining.map((city) => `<option>${escape(city)}</option>`).join('')}</select></label><label>节点记录说明<textarea name="note" rows="2" maxlength="300" required placeholder="如：已完成停车检查，宠物状态已记录"></textarea></label>`, '记录模拟到达节点')}</section>`;
    }
    if (f.stage === 'arrived') return `<section class="driver-action-card"><span class="driver-step">下一步 · 核对签收</span><h2>把毛孩子安心交到手中</h2>${form('receipt', `<label>实际签收人姓名<input name="receiverName" maxlength="40" required placeholder="请输入本次虚拟签收人" /></label>${checkbox('receiverVerified', '已核对签收人身份及交接信息')}${checkbox('petAccepted', '宠物交接情况已确认，无待处理异常')}`, '确认模拟签收')}</section>`;
    return '';
  }
  function detail(order) {
    ...
    main.innerHTML = `<section class="driver-detail-hero"><p>${escape(order.id)}</p><div>${icon(order.petType === '犬' ? 'dog' : 'cat')}<h1>${escape(order.petName)}的出行</h1></div><h2>${escape(order.fromCity)} → ${escape(order.toCity)}</h2><span class="fulfillment-chip">${escape(labels[f?.stage] || '待验宠')}</span></section>${nextAction(order)}${ui.markup(order, 'driver')}
      <section class="driver-contact-card"><h2>本次接送信息</h2><dl><div><dt>宠物资料</dt><dd>${escape(order.petType)} · ${escape(order.breed)} · ${escape(order.weight)}kg</dd></div><div><dt>接宠合作点</dt><dd>${escape(order.assignedNode?.name || '')}</dd></div><div><dt>司机 / 车辆</dt><dd>${escape(order.transport?.driverName)} · ${escape(order.transport?.vehiclePlate)}</dd></div><div><dt>寄件人</dt><dd>${escape(order.contactName)} · ${escape(order.contactPhone)}</dd></div><div><dt>收件人</dt><dd>${escape(order.recipientName)} · ${escape(order.recipientPhone)}<br>${escape(order.recipientAddress)}</dd></div><div><dt>照护说明</dt><dd>${escape(order.careNote || '暂无特殊照护说明')}</dd></div></dl></section>
      ${f && !['exception', 'delivered'].includes(f.stage) ? `<details class="driver-exception-form"><summary>${icon('alert-circle')}遇到问题？上报履约异常</summary><p>上报后暂停后续动作，由经营者处理。这里不会发送真实通知。</p>${form('exception', '<label>异常情况<textarea name="note" rows="3" maxlength="300" required placeholder="说明发生情况及当前需要处理的问题"></textarea></label>', '提交异常并暂停履约')}</details>` : ''}`;
    main.querySelectorAll('[data-action]').forEach((formNode) => {
      ...
      formNode.addEventListener('submit', async (event) => {
        ...
        try { const result = await api(`/api/driver/orders/${encodeURIComponent(order.id)}/${action}`, { method: 'POST', body }); state.orders = state.orders.map((o) => o.id === order.id ? result.order : o); toast('已记录本次虚拟操作'); render(); window.scrollTo(0, 0); }
  ...
  function profile() {
    main.innerHTML = `<div class="mini-profile-card"><span>${icon('van')}</span><div><h1>${escape(state.session.name)}</h1><p>${escape(state.session.account)}</p><small>司机权限 · 仅可操作自己的已派任务</small></div></div><div class="mini-menu-card"><div class="mini-version">总部所在地<span>安徽 · 合肥</span></div></div><section class="driver-contact-card"><h2>本轮演示说明</h2><p>到点验宠 → 用户确认费用 → 合作点离点 → 司机出发 → 节点登记 → 模拟签收。</p><p>异常由经营者处理，不会自动变为签收。切换账号前请退出；跨账号操作后点击刷新查看最新状态。</p></section><button id="driver-logout" class="outline-button mini-full-button">退出当前账号</button><p class="driver-demo-note">R3 · V5 清爽界面 · 本地虚拟履约</p>`;
