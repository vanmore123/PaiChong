(() => {
  ...
  function markup(order, role = 'user') {
    ...
    return `<section class="fulfillment-panel" aria-label="履约与尾款"><div class="fulfillment-heading"><h3>验宠与费用确认</h3><span class="fulfillment-chip">${invoice.status === 'paid' ? '已确认' : '待用户确认'}</span></div>
      <dl class="fulfillment-prices"><div><dt>司机确认总价</dt><dd>${money(invoice.total)}</dd></div><div><dt>已付保证金抵扣</dt><dd>− ${money(invoice.deposit)}</dd></div><div class="fulfillment-total"><dt>${invoice.status === 'paid' ? '已付尾款' : '待付尾款'}</dt><dd>${money(invoice.amount)}</dd></div></dl>
      ...
      <p class="fulfillment-footnote">到达时间按行程计划展示，登记时间为操作时间；不提供实时定位。</p></section>`;
  }
