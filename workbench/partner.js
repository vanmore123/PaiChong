(() => {
  ...
  const escape = value => String(copy(value == null ? '' : value)).replace(/[&<>"']/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[char]));
  ...
  function partnerView() {
    ...
    return '<section class="partner-hero"><div><span class="partner-eyebrow">合作机构 · ' + (handoffEnabled ? '交接与照护' : '容量管理') + '</span><h1>照顾好每一个<br />小客人</h1><p>' + escape(state.node ? state.node.name : '我的合作点') + '</p></div><img src="./assets/v5/brand/paichong-logo.png" alt="派宠宠物专车" /></section><p class="partner-note">' + (handoffEnabled ? '仅展示本合作点的交接订单、必要联系资料与容量申报。' : '仅展示当前机构的容量与申报记录，不含客户个人资料。') + '</p>' + errorBox(state.error) + body;
  }
